...
module {
  ...
  func beBytes(n: Nat32) : [Nat8] {
   ...
   public func getDefaultAccountText(principal: Principal): Text{
        let subaccountBlob : SubaccountBlob = Utils.defaultSubaccount();
        let accountIdText = Utils.accountIdToHex(accountIdentifier(principal, subaccountBlob));
        return accountIdText;
    };

   public  func getAccountTextId(principal: Principal,sub: Nat): Text{
        let subaccountBlob : SubaccountBlob = Utils.subToSubBlob(sub);
        let accountIdText = Utils.accountIdToHex(accountIdentifier(principal, subaccountBlob));
        return accountIdText;
    };
  ...
  public func accountIdentifier(principal: Principal, subaccount: SubaccountBlob) : AccountIdentifier {
